Vagrant.configure("2") do |config|
  ...
  config.vm.define "rancher-host1" do |rancherhost|
    ...
    config.vm.provision "create-dev", type:'ansible' do |ansible|
      ansible.config_file = 'ansible.cfg'
      ansible.limit = "all"
      ...
      ansible.raw_arguments = ["-e NAME_PROJECT=dev"]
    end
  end

    config.vm.define "rancher-host2" do |rancherhost|
    rancherhost.vm.box = "ubuntu/xenial64"
    rancherhost.vm.network "private_network", ip: "192.168.100.30", virtualbox__intnet: true
    rancherhost.vm.hostname = "rancher-host2"
    rancherhost.vm.provider "virtualbox" do |vb|
      vb.memory = 768
    end
    config.vm.provision "create-stage", type:'ansible' do |ansible|
      ansible.config_file = 'ansible.cfg'
      ansible.limit = "all"
      ansible.playbook = 'create_project.yml'
      ansible.groups = {
      ...
      ansible.raw_arguments = ["-e NAME_PROJECT=stage"]
    end
  end

    config.vm.define "rancher-host3" do |rancherhost|
    rancherhost.vm.box = "ubuntu/xenial64"
    rancherhost.vm.network "private_network", ip: "192.168.100.30", virtualbox__intnet: true
    rancherhost.vm.hostname = "rancher-host3"
    rancherhost.vm.provider "virtualbox" do |vb|
      vb.memory = 768
    end
    config.vm.provision "create-prod", type:'ansible' do |ansible|
      ansible.config_file = 'ansible.cfg'
      ansible.limit = "all"
      ansible.playbook = 'create_project.yml'
      ansible.groups = {
      ...
      ansible.raw_arguments = ["-e NAME_PROJECT=prod"]
    end
  end
